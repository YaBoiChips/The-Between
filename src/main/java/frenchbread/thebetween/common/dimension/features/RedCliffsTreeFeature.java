...
public class RedCliffsTreeFeature extends Feature<BaseTreeFeatureConfig> {

    public RedCliffsTreeFeature(Codec<BaseTreeFeatureConfig> codec) {
        super(codec);
    }
    ...
    private static boolean isNetherrackOrHellstoneAt(IWorldGenerationBaseReader reader, BlockPos pos) {
        return reader.hasBlockState(pos, (state) -> {
    ...
    private boolean place(IWorldGenerationReader generationReader, Random rand, BlockPos positionIn, Set<BlockPos> logPositions, Set<BlockPos> foliagePositions, MutableBoundingBox boundingBoxIn, BaseTreeFeatureConfig configIn) {
        ...
        if (blockpos.getY() >= 1 && blockpos.getY() + i + 1 <= 256) {
            if (!isNetherrackOrHellstoneAt(generationReader, blockpos.down())) {
                return false;
            } else {
